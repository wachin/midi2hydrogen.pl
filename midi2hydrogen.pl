...
--dump
   Dumps the whole content of the midi file on stdout. This helps in troubleshooting
...
sub notemap		{ return _field_("notemap",@_)		}
...
sub addNote
{
	...
	my($key)="$position:$h2ins";
	my($map)=$self->notemap();
	my($note);

	if(defined($note=$map->{$key}))
	{
		# Note duplication (same position, same instrument)
		# => keep the highest velocity
...
sub dump
{
	...
	printf("Hygrogen song dump:\n");
	printf("===================\n\n");
	my($str)="  +-------+-----+-------------------------------------+------------------------------------+\n";
	printf($str);
	printf("  | Pos   | Vel | Hydrogen Instrument                 | Midi instrument                    |\n");
	printf($str);
	foreach my $note (@{$self->notes()})
	{
		printf("  | %05d | %-3s | (%02d) %-30s | (%02d) %-30s|\n",
			$note->{position},
			$note->{velocity},
			$note->{instrument},$self->h2InstrumentName($note->{instrument}),
			$note->{midinum},$self->midiInstrumentName($note->{midinum}),
		);
	}
	printf($str);
}

sub h2InstrumentName
{
	my($self)=shift;
	my($num)=shift;

	my($ins);
	if(defined($ins=$self->instruments()->[$num]))
	{
		return $ins->{name};
	}
	return "(unknown)";
...
sub midiInstrumentName
{
	my($self)=shift;
	my($num)=shift;

	return $MIDI::notenum2percussion{$num};
}
...
sub asString
{
	...
	my(@inslist)=@{$self->instruments()};
	for my $insnum (0...31)
	{
		my($ins)=$inslist[$insnum];
	...
	foreach my $pat (@{$self->patterns()})
	{
		...
		foreach my $seq (@{$pat->{sequences}})
		{
			...
			{
				push(@list,"<note>");
				foreach my $field (keys(%Cfg::DFLNOTE))
				{
					push(@list,"<$field>$note->{$field}</$field>");
				}
				push(@list,"</note>");
			}
...
sub ShowSongInfo
{
	...
	my(@tracks)=$midisong->tracks();
	my($i)=0;
	my($ev);

	my($ticks)=$midisong->ticks();
...
my($midisong)=eval { MIDI::Opus->new({ from_file => $midiFile }) };
unless($midisong)
{
...
ProcessSong($midisong);

if($OPTS{dump})
{
	ShowSongInfo($midisong);
	$SONG->dump();
	print "\nMIDI FILE DUMP:\n";
	print "--------------\n\n";
	$midisong->dump({ dump_tracks => 1, Eflat => 1 });
	exit(0);
}
